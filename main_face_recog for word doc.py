...
class Login:
	...
	def login_check(self):
		...
		if login_block_time > current_time: #Checks whether the login_block_time is after the current time
			messagebox.showinfo("","Access denied, you cannot login for another minute")
...
class Dashboard:
	...
	def __init__(self, master):
		...
		detection_list_query = self.db.execute("SELECT Profile_Name, Profile_DetectTime FROM Profile WHERE Profile_DetectTime IS NOT NULL")
		detection_list_query = detection_list_query.fetchall()
		self.detection_list_array = []
		for person in detection_list_query:
			for element in person:
				self.detection_list_array.append(element)
		
		class_list = [] #Defines array to hold the names of profiles
		for name in name_list_query: #Iterates through each tuple in the array
			class_list.append(name[0]) #Adds first element of each tuple to the name_list array
		...
		self.menubar.add_command(label="Train model", command=lambda: Thread(target=self.train_model_button()).start())
	...
	def update_detection_list(self, name):
		print(self.detection_list_array)
		try:
			index_of_name = self.detection_list_array.index(name)
			self.detection_list_array.pop(index_of_name) #Removes name of person from detection_list_array
			self.detection_list_array.pop(index_of_name) #Removes the time associated with the person detected
		except:
			pass

	def detection_list_check(self):
		threading.Time(5.0, self.detection_list_check).start()
		current_time = datetime.now().strftime("%H:%M:%S")
		for time_index in range(0, int(len(self.detection_list_array)/2)):
			time_index = (time_index*2)+1
			detect_by_time = datetime.strptime(self.detection_list_array[time_index], "%H:%M:%S").strftime("%H:%M:%S")
			if detect_by_time < current_time:
				self.send_message(self.detection_list_array[time_index-1], "not-arrived")


	def send_message(self, name, state):
		connection = sqlite3.connect('database.db') #Establishes a connection to the database
		db = connection.cursor()
		send_to = db.execute("SELECT Setting_Value FROM Settings WHERE Setting = ?", ("Phone_Number",)) #Retrieves the phone number from the database
		send_to = send_to.fetchall()[0][0]
		if state == "not-arrived": #If message to be sent needs to be sent regarding a person not arriving by a certain time
			message = name + " has not arrived yet" #Constructs the message
		else:
			category = db.execute("SELECT Profile_Category FROM Profile WHERE Profile_Name = ?", (name,)) #Retrieves the category of the detected person
			category = category.fetchall()[0][0] 
			message = "Kevin Hart has not arrived yet "
			#message = name + " was detected now, this person is classified as " + category #Constructs the message for a person being detected
		
		
		from twilio.rest import Client #Imports the Twillio Module
		#Declares the credentials relating to the Twillio account
		account_sid = "AC976b9602e9dcb0092adcb342a009ac28"
		auth_token = "7dc9faf2af8814941cc4e56e03367e4a"
		client = Client(account_sid, auth_token)
		message = client.messages.create(
		        to=send_to,
		        from_="+12055095032",
		        body=message)
		print(message.sid) #Sends the message to the user
	...
	def show_frame(self):
		...
		self.record_start = False #Sets a boolean variable to store whether recording has started or not
		no_face_frames = 0 #Stores the number of consecutive frames without a face 
		_, frame = self.cap.read() #Reads frame from camera
		face = self.face_extractor(frame) #Extracts a face from the frame if a face is present
		width = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH) + 0.5) #Gets the width of the video
		height = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT) + 0.5) #Gets the height of the video
		while True:
			try:
				_, frame = self.cap.read() #Reads frame from camera
				original_frame = frame
				face = self.face_extractor(frame) #Extracts face from the frame if present
				if type(face) is np.ndarray: #If a face has been extracted 
					face = cv2.resize(face, (224, 224)) #Resizes face
					face = Image.fromarray(face, 'RGB') #converts image format
					face_array = np.array(face) #Converts image to numpy array
					face_array = np.expand_dims(face_array, axis=0)
					prediction = model.predict(face_array) #Gets prediction from model of who the face belongs to
					y_classes = prediction.argmax(axis=-1) #Gets the class with the highest prediction
					name = class_list[int(y_classes)] #Extracts the name for the class with the highest prediction
					cv2.putText(frame, name, (50,50), cv2.FONT_HERSHEY_COMPLEX, 1, (0,255,0), 2) #Displays name of person in the video frame. 
					no_face_frames = 0 #Resets variable to 0
					if self.record_start == False: #If recording has not already started
						start = datetime.today() #Retrives the current date abd time
						start_date = start.strftime("%Y-%m-%d") #Gets the current date
						start_time = start.strftime("%H:%M:%S") #Gets the current time
						next_log_id = db.execute("SELECT seq FROM sqlite_sequence WHERE name = ?", ("Log",)) #Extracts the next record_id
						next_log_id = next_log_id.fetchall()[0][0] 
						fourcc = cv2.VideoWriter_fourcc('m','p','4','v') #Defines the encoder to be used
						video_record = cv2.VideoWriter("videos/"+str(int(next_log_id)+1)+".mp4", fourcc, 20.0, (width,height)) #Defines the recording object 
						self.record_start = True #Sets record_start variable to True to indicate recording has started			
						self.send_message(name, "detection")
						self.update_detection_list(name)
					
				else:
					no_face_frames += 1 #If no face is detected in the frame

				if no_face_frames > 40 and self.record_start == True: #If no faces have been detected for 10 frames in a row
					video_record.release() #Stops recording
					end = datetime.now() #Extracts the time when recoridng has ended
					end_time = end.strftime("%H:%M:%S")
					#Retrieves profile_id of the person that has been detected
					profile_id = db.execute("SELECT Profile_Id FROM Profile WHERE Profile_Name=?", (name, )) 
					profile_id = profile_id.fetchall()[0][0]
					#Inserts the details of the log into the database
					sql = ("INSERT INTO Log (Log_ProfileId, Log_Date, Log_StartTime, Log_EndTime, Log_FaceReview) Values (?, ?, ?, ?, ?)")
					values =  (profile_id, start_date, start_time, end_time, 0) #Defines variables to be subsituted in as parameters
					db.execute(sql, values) 	
					connection.commit()
					no_face_frames = 0 #Resets no_face counter variable to 0
					self.record_start = False #Resets the recoridng start variable
				else:
					try:
						video_record.write(original_frame) #if a face has been detected, it will write the frame to the current video
					except:
						pass
				...
				pass
				messagebox.showinfo("","Camera disconnected, please reconnect and restart program")
				self.master.destroy()
	...
	def face_extractor(self, frame):
		#Checks whether a face is present in the frame using the haarcascade
		faces = face_cascade.detectMultiScale(frame, 1.3, 5)

		#If no faces are present
		if faces is ():
			return None

		# Crop all faces found
		for (x,y,w,h) in faces:
			cv2.rectangle(frame, (x,y),(x+w,y+h),(0,255,255),2) #Puts a rectangle around the face in the frame 
			cropped_face = frame[y:y+h, x:x+w] #Crops the face from the whole frame
...
class Profile_Form():
	...
	def load_profile(self):
		...
		if name not in self.name_list: #Checks whether the name exists in the database
			messagebox.showerror("","Profile not found", parent=self.master) 
		else:
	...
	def save(self, name, arrive_by, category, open_door):
		...
		if len(name) < 2: #Checks if name entry is 2 or fewer characters long
			...
			messagebox.showinfo("","Profile saved", parent=self.master)
...
class Report_Form():
	...
	def export(self):
		...
		if len(self.result_list) == 0: #If no records to export
			...
			directory = filedialog.asksaveasfilename(defaultextension = "xls", parent=self.master)
	...
	def search(self):
		...
		if log_id != "": #Checks if a log id has been entered
			try:
				...
				log_id_valid = False
				messagebox.showerror("","Please enter a valid log id", parent=self.master)
		...
		#If no log id has been entered, the search will be based off the profile id
			...
			if name not in self.database_name_list:
				messagebox.showerror("","Profile with name entered does not exist", parent=self.master)
			else:
		...
		if date_1 == "" and date_2 == "": #If both date fields are empty
			...
			if date_1_valid == True and date_2_valid == True: 
				...
				messagebox.showerror("","Please enter a valid date", parent=self.master)
		...
		if len(log_result_list_raw) == 0: #If no records match the criteria
			messagebox.showinfo("", "No logs found", parent=self.master)
			self.result_list = []
...
def train_model():
	...
	connection = sqlite3.connect('database.db') #Establishes a connection to the database
	db = connection.cursor()
	#Retrieves the profile names from the database
	name_list_query = db.execute("SELECT Profile_Name FROM Profile") 
	name_list_query = name_list_query.fetchall() 
	name_list = [] #Defines array to hold the names of profiles
	for name in name_list_query: #Iterates through each tuple in the array
		name_list.append(name[0]) #Adds first element of each tuple to the name_list array
	if len(name_list) == 0:
		pass
	else:
		#Defines size of image
		image_size = [224, 224]
		vgg = VGG16(input_shape=image_size + [3], weights='imagenet', include_top=False) #Defines the neural network to be used
		#Configures the last layer of the neural network
		for layer in vgg.layers:
			layer.trainable = False
		x = Flatten()(vgg.output)
		prediction = Dense(len(name_list), activation='softmax')(x)
		model = Model(inputs=vgg.input, outputs=prediction) #Compiles the model which will be used to train the model
		model.summary()
		model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

		#Declares the data augmentation to be applied to the images
		train_datagen = ImageDataGenerator(rescale = 1./255, shear_range = 0.2, zoom_range = 0.2, horizontal_flip = True) 
		test_datagen = ImageDataGenerator(rescale = 1./255)
		
		#Calculates the total number of the images in each profile folder
		total = 0
		...
		#Calculates appropriate batch size
		if total>32:
			batch_size = 32
		elif total > 10:
			batch_size = 8
		...
		training_set = train_datagen.flow_from_directory('profiles', target_size = image_size, batch_size = batch_size,
		class_mode = 'categorical', classes = name_list)
		test_set = test_datagen.flow_from_directory('profiles', target_size = image_size, batch_size = batch_size, 
		class_mode = 'categorical', classes = name_list)
		
		train_model = model.fit_generator(training_set, validation_data=test_set, epochs=5, 
		steps_per_epoch=len(training_set), validation_steps=len(test_set)) 

		#Checks if the accuracy of the last epoch is greater than 0.90
		if train_model.history['acc'].pop() > 0.90:
			model.save('facial_recognition_model.h5') #Saves model 
		else:
			train_model()
